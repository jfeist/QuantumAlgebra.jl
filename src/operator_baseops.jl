...
comm(A::OpSum,B::Operator)      = sum(sA*comm(tA,B) for (tA,sA) in A.terms)
comm(A::OpSum,B::OpProd)        = invoke(comm,Tuple{OpSum,Operator},A,B)
comm(A::OpSum,B::OpSumAnalytic) = invoke(comm,Tuple{OpSum,Operator},A,B)
comm(A::Operator,     B::OpSum) = sum(sB*comm(A,tB) for (tB,sB) in B.terms)
comm(A::OpProd,       B::OpSum) = invoke(comm,Tuple{Operator,OpSum},A,B)
comm(A::OpSumAnalytic,B::OpSum) = invoke(comm,Tuple{Operator,OpSum},A,B)
comm(A::OpSum,B::OpSum) = sum((sA*sB)*comm(oA,oB) for (oA,sA) in A.terms for (oB,sB) in B.terms)
